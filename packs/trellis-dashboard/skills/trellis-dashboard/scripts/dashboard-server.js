...
function listResearch(taskDir) {
  ...
  return fs.readdirSync(dir, { withFileTypes: true })
    ...
    .map((entry) => ({ name: entry.name, content: readText(path.join(dir, entry.name)) }));
}
...
function readSessions(repoRoot) {
  ...
  return fs.readdirSync(dir, { withFileTypes: true })
    ...
    .map((entry) => ({ id: entry.name, data: readJson(path.join(dir, entry.name), {}) }));
}
...
function buildState(repoRoot) {
  ...
  return {
    ...
    archivedTasks,
    events,
    sessions: sessions.map((session) => ({ id: session.id, taskPath: session.data.task_path || '', source: session.data.source || '' })),
    summary: { activeCount: tasks.length, archivedCount: archivedTasks.length, byStatus },
...
function dashboardRuntimeDir(repoRoot) { return path.join(repoRoot, '.trellis', '.runtime', 'dashboard'); }
function dashboardRecordPath(repoRoot) { return path.join(dashboardRuntimeDir(repoRoot), 'server.json'); }
function createServer(repoRoot, host, port) {
  ...
  const watchers = [path.join(repoRoot, '.trellis', 'tasks'), path.join(repoRoot, '.trellis', '.runtime')]
    .filter((target) => fs.existsSync(target))
    .map((target) => fs.watch(target, { recursive: true }, () => broadcast(path.relative(repoRoot, target).replace(/\\/g, '/'))));
  const server = http.createServer((req, res) => {
    const url = new URL(req.url || '/', `http://${host}:${port}`);
    if (url.pathname === '/api/overview') return sendJson(res, 200, state);
    if (url.pathname === '/api/tasks') return sendJson(res, 200, { tasks: state.tasks, archivedTasks: state.archivedTasks, summary: state.summary });
    ...
    if (url.pathname === '/api/current-session') return sendJson(res, 200, { currentTask: state.currentTask, currentAgent: state.currentAgent, sessions: state.sessions });
    ...
    const asset = url.pathname === '/' ? path.join(WEB_ROOT, 'index.html') : path.join(WEB_ROOT, url.pathname.replace(/^\//, ''));
    if (!asset.startsWith(WEB_ROOT) || !fs.existsSync(asset) || fs.statSync(asset).isDirectory()) return sendText(res, 404, 'not found');
...
function startCommand(args) {
  ...
  if (!repoRoot) { console.error('No .trellis directory found from project root'); process.exit(1); }
  ...
  if (record.pid) {
    try {
      ...
      console.log(JSON.stringify({ status: 'already-running', url: record.url, pid: record.pid }));
      if (shouldOpen) openBrowser(record.url);
      return;
    } catch {}
  }
  if (!foreground) {
    const child = spawn(process.execPath, [__filename, 'start', '--project', repoRoot, '--host', host, '--port', String(port), '--foreground'], { detached: true, stdio: 'ignore' });
    child.unref();
    const url = `http://${host}:${port}`;
    console.log(JSON.stringify({ status: 'starting', url, pid: child.pid }));
    if (shouldOpen) setTimeout(() => openBrowser(url), 400);
    return;
  }
  ...
  server.listen(port, host, () => {
    ...
    fs.writeFileSync(dashboardRecordPath(repoRoot), JSON.stringify({ pid: process.pid, host, port, url, repoRoot, startedAt: new Date().toISOString() }, null, 2), 'utf8');
    console.log(JSON.stringify({ status: 'running', url, pid: process.pid, repoRoot }));
    if (shouldOpen) openBrowser(url);
  });
  const cleanup = () => {
    try { fs.rmSync(dashboardRecordPath(repoRoot), { force: true }); } catch {}
...
function openCommand(args) {
  ...
  if (!repoRoot) { console.error('No .trellis directory found from project root'); process.exit(1); }
  const record = readJson(dashboardRecordPath(repoRoot), {});
  const url = record.url || `http://${DEFAULT_HOST}:${DEFAULT_PORT}`;
  openBrowser(url);
  console.log(JSON.stringify({ status: 'opened', url }));
}
function stopCommand(args) {
  const repoRoot = findRepoRoot(args.project || process.cwd());
  if (!repoRoot) { console.error('No .trellis directory found from project root'); process.exit(1); }
  const record = readJson(dashboardRecordPath(repoRoot), {});
  if (record.pid) { try { process.kill(record.pid); } catch {} }
  try { fs.rmSync(dashboardRecordPath(repoRoot), { force: true }); } catch {}
  console.log(JSON.stringify({ status: record.pid ? 'stopped' : 'not-running', pid: record.pid || null }));
}
const args = parseArgs(process.argv.slice(2));
const command = args._[0] || 'start';
if (command === 'start') startCommand(args);
else if (command === 'open') openCommand(args);
else if (command === 'stop') stopCommand(args);
